...
for (const p of data) {
  ...
  if (p.affiliate_status === 'approved' && !['generated_original_placeholder', 'generated_fallback', 'sitestripe', 'approved_real', 'merchant_feed', 'paapi', 'licensed', 'site_verified', 'approved_affiliate_or_licensed'].includes(p.image_status)) bad.push(`${p.id}: unsafe image status ${p.image_status}`);
}
