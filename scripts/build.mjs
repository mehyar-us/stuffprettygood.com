...
// RSS 2.0 feed of the /guides/<slug>/ buying-guide posts. Newest-first by modified_at.
const _spgFeedItems = posts.slice().sort((a, b) => {
  const am = Date.parse(a.modified_at || a.published_at || '1970-01-01') || 0;
  const bm = Date.parse(b.modified_at || b.published_at || '1970-01-01') || 0;
  return bm - am;
}).map((post) => {
  const postUrl = `https://stuffprettygood.com/guides/${post.slug}/`;
  const pubDate = new Date(Date.parse(post.published_at || post.modified_at || new Date().toISOString()) || Date.now()).toUTCString();
  const desc = (post.intro || post.title || '').slice(0, 280);
  return `<item><title>${esc(post.title)}</title><link>${postUrl}</link><guid isPermaLink="true">${postUrl}</guid><pubDate>${pubDate}</pubDate><description><![CDATA[${desc}]]></description><author>noreply@stuffprettygood.com (${esc(post.author || 'Stuff Pretty Good Editorial')})</author><category>${esc(post.category || 'general')}</category></item>`;
}).join('\n');
const _spgFeedLastBuild = new Date().toUTCString();
fs.writeFileSync(path.join(dist, 'feed.xml'), `<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom"><channel><title>Stuff Pretty Good — Buying Guides</title><link>https://stuffprettygood.com/guides/</link><description>Practical buying guides with hand-picked, approved-affiliate products for everyday problems. New guides weekly.</description><language>en-us</language><lastBuildDate>${_spgFeedLastBuild}</lastBuildDate><atom:link href="https://stuffprettygood.com/feed.xml" rel="self" type="application/rss+xml"></atom:link>\n${_spgFeedItems}\n</channel></rss>\n`);
