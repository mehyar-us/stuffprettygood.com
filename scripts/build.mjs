...
for (const route of categories.filter((r) => !['gift-finder', 'starter-kits', 'useful-finds'].includes(r))) {
  ...
  const liveSection = isTravel
    ...
    : isWalmart
      ? `<section class="section"><div class="section-head"><div><p class="eyebrow">Impact catalog</p><h2>Fresh Walmart picks</h2></div></div><div class="grid" data-live-picks><p class="micro">Loading approved Walmart picks…</p></div>${liveDailyPicksScript('', 'walmart')}</section>`
      : '';
...
mkdirPage('useful-finds', layout('Useful Finds', `<section class="section"><p class="eyebrow">Useful picks</p><h1>Useful Finds</h1><p class="sub">Browse useful upgrades for gifts, home, kitchen, travel, tech, pets, and everyday problems.</p><div class="section-head"><div><p class="eyebrow">Fresh daily picks</p><h2>Newest from the live catalog</h2></div></div><div class="grid" data-live-picks><p class="micro">Loading daily picks…</p></div>${liveDailyPicksScript()}<div class="section-head"><div><p class="eyebrow">Walmart via Impact</p><h2>Approved Walmart catalog picks</h2></div><a class="pill" href="/walmart/">Browse Walmart</a></div><div class="grid" data-live-picks="walmart"><p class="micro">Loading Walmart picks…</p></div>${liveDailyPicksScript('', 'walmart', '[data-live-picks="walmart"]')}<h2>Full launch catalog</h2><div class="grid">${products.map(card).join('')}</div></section>`));
...
function liveDailyPicksScript(category = '', merchant = '', selector = '[data-live-picks]') {
  ...
  fetch('https://stuffprettygood-api.mehyar.workers.dev/api/catalog${query}')
    ...
    .then(function(data){
      ...
      mount.innerHTML = products.map(function(p){
        const source = ${JSON.stringify(merchant)} === 'walmart' ? '<div class="value-chip">Walmart · approved catalog</div>' : '';
        return '<article class="card"><a class="thumb" href="https://stuffprettygood-api.mehyar.workers.dev/products/'+encodeURIComponent(p.id)+'"><img src="'+esc(p.image_url)+'" alt="'+esc(p.title)+'"></a>'+source+'<div class="card-meta"><span>'+esc(String(p.price_band).replace('-', ' $'))+'</span><span>'+esc(String(p.category).replace('-', ' '))+'</span></div><h3>'+esc(p.title)+'</h3><p>'+esc(p.why_useful)+'</p><p class="best"><strong>Best for:</strong> '+esc(p.best_for)+'</p><a class="btn small" href="https://stuffprettygood-api.mehyar.workers.dev/products/'+encodeURIComponent(p.id)+'">Get</a></article>';
      }).join('');
    })
...
const urls = ['', 'gift-finder', 'starter-kits', 'under-25', 'under-50', 'walmart', 'useful-finds', 'travel', 'home-office', 'kitchen', 'pets', 'tech', 'signup', 'about', 'advertise', 'affiliate-disclosure', 'privacy', 'terms', 'contact', 'unsubscribe', 'preferences', ...posts.map((p) => 'guides/' + p.slug), ...products.map((p) => 'products/' + p.id)];
