...
(function(){
  ...
  bindSuggestions();
  // Delegated click handler for follow-up chips rendered inside bot messages.
  // Uses event delegation on [data-ai-messages] so chips added dynamically
  // (after each submit) are picked up without re-binding. Mirrors the haptic +
  // auto-submit pattern from bindSuggestions() above.
  const bindFollowups = function(){
    messages.addEventListener('click', function(e){
      const t = e.target;
      if (!t || !t.closest || !t.closest('[data-ai-followup]')) return;
      const btn = t.closest('[data-ai-followup]');
      haptic([10, 40, 10]);
      form.q.value = btn.textContent;
      form.dispatchEvent(new Event('submit', {cancelable:true}));
    });
  ...
  // Lane C #9 (tick 52): "Was this helpful?" feedback handler. Delegated
  // click on the messages container reads the vote from data-ai-feedback-vote
  // and toggles history[i]._fb. Persists via save() (already saves the
  // history blob which carries _fb on the bot entry). Re-evaluates
  // aria-pressed + active class on BOTH buttons in the row so the user
  // sees the chosen one light up and the other dim. Pure UX — no analytics
  // fired, no PII collected, opt-in only by the act of clicking.
  const bindFeedback = function(){
    messages.addEventListener('click', function(e){
      var t = e.target;
      ...
      // Toggle: clicking the active vote clears it; clicking the other flips it.
      m._fb = (m._fb === clicked) ? '' : clicked;
      save();
      // Update aria-pressed + active class on both buttons in this row.
  ...
  function renderHistory(){
    ...
    history.forEach(function(m, i){
      add(m.role, m.html);
      // Lane C #9 (tick 52): re-attach feedback row on history rebuild
      // so a persisted bot message keeps its thumbs-up/down state across
  ...
  function recommend(q, n=4){ return products.map(p => ({...p, score: score(p,q)})).filter(p => p.score > -5).sort((a,b)=>b.score-a.score).slice(0,n); }
    // Lane C backlog #2 (tick 12): follow-up chips after each bot response.
    // answer() now returns { html, kind } so the submit handler can attach a
    // contextual micro-question row. Kinds match the branches below — each gets
    // a small set of chips tuned to the shape of the answer (product shortlist
    ...
    // duplicating the question the user just asked (would feel like a robot).
    const FOLLOW_UPS_BY_KIND = {
    ...
    // element so the delegated click handler can find all follow-up chips
    // regardless of which bot message they belong to.
    function appendFollowUps(row){
      const chips = followUpsFor(row.kind);
      ...
      messages.lastElementChild && messages.lastElementChild.appendChild(wrap);
    }
    // Lane C #9 (tick 52): "Was this helpful?" feedback chips. Each bot
    // message gets a thin row of (👍 / 👎 / "Was this helpful?" prompt / hidden
    // thanks). Toggling a button writes the vote onto the history entry's
    // _fb field, persists to localStorage via the existing save() call,
    // and updates aria-pressed + the active-state CSS class. The vote
    // re-applies on renderHistory() rebuilds (see forEach loop above).
    // Pure UX — no PII collected, no analytics fired.
    function feedbackKey(q){
      // djb2 hash of the question text → 8-char fingerprint
      var h = 5381;
    ...
    function appendFeedbackRow(idx){
      ...
      // Append inside the bot message div (last child of messages).
      var botEl = messages.lastElementChild;
      if (botEl) botEl.appendChild(wrap);
      // Also expose the question fingerprint on the wrapper so the click
      // handler can confirm the key without re-reading history.
      wrap.setAttribute('data-ai-feedback-key', feedbackKey(q));
    }
  // Haptic feedback helper — returns instantly on unsupported browsers / iOS Safari
  // (navigator.vibrate is undefined). Soft 10ms pulse on launch, double-pulse on
  // suggestion chips (legible as a "tap pattern"), single-tick on send.
  // Respects prefers-reduced-motion: no haptic at all.
  ...
  form.addEventListener('submit', function(e){
    ...
    const user = { role:'user', html: esc(q) };
        const resp = answer(q);
        const bot = { role:'bot', html: resp.html, _q: q };
