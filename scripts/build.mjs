...
<section class="section"><div class="section-head"><div><p class="eyebrow">Fresh daily picks</p><h2>New useful finds from the live catalog</h2></div><a class="pill" href="/useful-finds/">See more</a></div><div class="grid product-wall" data-live-picks><p class="micro">Loading today’s fresh picks…</p></div></section>${liveDailyPicksScript('', '', '[data-live-picks]', 48)}
...
mkdirPage('useful-finds', layout('Useful Finds', `<section class="section"><p class="eyebrow">Useful picks</p><h1>Useful Finds</h1><p class="sub">Browse useful upgrades for gifts, home, kitchen, travel, tech, pets, and everyday problems.</p><div class="section-head"><div><p class="eyebrow">Fresh daily picks</p><h2>Newest from the live catalog</h2></div></div><div class="grid product-wall" data-live-picks><p class="micro">Loading daily picks…</p></div>${liveDailyPicksScript('', '', '[data-live-picks]', 60)}<div class="section-head"><div><p class="eyebrow">Walmart via Impact</p><h2>Approved Walmart catalog picks</h2></div><a class="pill" href="/walmart/">Browse Walmart</a></div><div class="grid product-wall" data-live-picks="walmart"><p class="micro">Loading Walmart picks…</p></div>${liveDailyPicksScript('', 'walmart', '[data-live-picks="walmart"]', 60)}<h2>Full launch catalog</h2><div class="grid">${products.map(card).join('')}</div></section>`));
mkdirPage('stories', layout('AI Shopping Stories', `<section class="section stories-page"><p class="eyebrow">Daily AI shopping stories</p><h1>Story-driven shopping lists</h1><p class="sub">SPG creates daily real-life scenarios — hiking, camping, emergency prep, travel, sporting days, road trips, home resets, pet weekends, and more — then builds shopping lists only from approved products with images and monetized /go links.</p><div class="story-wall" data-live-stories><p class="micro">Loading today’s stories…</p></div>${liveStoriesScript(20)}</section>`));
...
(function(){
  ...
  fetch('https://stuffprettygood-api.mehyar.workers.dev/api/stories?limit=${String(limit)}')
    ...
    .then(function(data){
      ...
      mount.innerHTML = stories.map(function(story){
        const items = (story.checklist || []).slice(0, 7).map(function(item){
          return '<a class="story-item" href="'+esc(item.product_url)+'"><img src="'+esc(item.image_url)+'" alt="'+esc(item.title)+'" loading="lazy"><span><b>'+esc(item.title)+'</b><small>'+esc(item.category || '')+' · Get</small></span></a>';
        }).join('');
        return '<article class="story-card"><div class="story-kicker">'+esc(story.theme).replace(/-/g,' ')+'</div><h2>'+esc(story.title)+'</h2><p>'+esc(story.story_text || story.situation)+'</p><div class="story-items">'+items+'</div></article>';
      }).join('');
    })
  ...
  fetch('https://stuffprettygood-api.mehyar.workers.dev/api/catalog${query}')
    ...
    .then(function(data){
      ...
      mount.innerHTML = products.map(function(p){
        ...
        return '<article class="card"><a class="thumb" href="https://stuffprettygood-api.mehyar.workers.dev/products/'+encodeURIComponent(p.id)+'"><img src="'+esc(p.image_url)+'" alt="'+esc(p.title)+'" loading="lazy"></a>'+source+'<div class="card-meta"><span>'+esc(String(p.price_band).replace('-', ' $'))+'</span><span>'+esc(String(p.category).replace('-', ' '))+'</span></div><h3>'+esc(p.title)+'</h3><p>'+esc(p.why_useful)+'</p><p class="best"><strong>Best for:</strong> '+esc(p.best_for)+'</p><a class="btn small" href="https://stuffprettygood-api.mehyar.workers.dev/products/'+encodeURIComponent(p.id)+'">Get</a></article>';
      }).join('');
    })
