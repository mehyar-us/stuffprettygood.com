...
function card(p, i = 0) {
  return `<article class="card" style="--delay:${i % 6}"><a class="thumb" href="/products/${p.id}/"><img src="${p.image_url}" alt="${esc(p.title)} illustrated fallback"></a><div class="real-photo-note">View product photos on the pick page.</div><div class="card-meta"><span>${esc(p.price_band.replace('-', ' $'))}</span><span>${esc(p.category.replace('-', ' '))}</span></div><h3>${esc(p.title)}</h3><p>${esc(p.why_useful)}</p><p class="best"><strong>Best for:</strong> ${esc(p.best_for)}</p><a class="btn small" href="/products/${p.id}/">See the pick</a></article>`;
}
...
for (const route of categories.filter((r) => !['gift-finder', 'starter-kits', 'useful-finds'].includes(r))) {
  ...
  mkdirPage(route, layout(titleCase(route), `<section class="section"><p class="eyebrow">Approved catalog</p><h1>${titleCase(route)}</h1><p class="sub">Practical Amazon picks organized by budget, job, and real-life usefulness.</p><div class="grid">${picks.map(card).join('')}</div></section>`));
}
...
const policyPages = {
  ...
  'affiliate-disclosure': `<h1>Affiliate Disclosure</h1><p class="sub">Stuff Pretty Good uses affiliate relationships to keep the guide free for shoppers.</p><p>As an Amazon Associate, Stuff Pretty Good earns from qualifying purchases.</p><p>Our goal is simple: explain why a pick is useful, who it fits, and when to skip it.</p>`,
