...
(function(){
  ...
  bindFeedback();
    ...
    document.addEventListener('click', function(e){
      ...
      if (!anchor) return;
      var m = (anchor.getAttribute('href') || '').match(new RegExp('^/products/([^/]+)/?$'));
      if (!m) return;
  ...
  const bindVerdict = function(){
    ...
    verdictForm.addEventListener('submit', function(e){
      ...
      const stripped = text.replace(new RegExp('^https?:\\/\\/(www\\.)?'), '').replace(new RegExp('\\/+$'), '').slice(0, 240);
  ...
  function recordShareState(q, botHtml){
    ...
    var m = String(botHtml || '').match(new RegExp('/products/([a-z0-9-]+)/', 'g')) || [];
    var ids = [];
    m.forEach(function(h){ var id = h.replace(new RegExp('/products/'), '').replace(new RegExp('/$'), ''); if (ids.indexOf(id) === -1) ids.push(id); });
  ...
  function recommend(q, n=4){ return products.map(p => ({...p, score: score(p,q)})).filter(p => p.score > -5).sort((a,b)=>b.score-a.score).slice(0,n); }
    ...
    function answer(q){
      ...
      if (/^verdict on\s+/.test(low)){
        const target = q.replace(new RegExp('^verdict on\\s+', 'i'), '').trim();
